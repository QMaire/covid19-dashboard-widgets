...
<template>
  ...
  <div>
    ...
    <div class="list_container">
      ...
      <select class="rf-select" id="select-reg" name="select" @change="changeGeoLevel('reg',$event)">
    ...
    <div class="list_container">
      ...
      <select class="rf-select" id="select-dep" name="select" @change="changeGeoLevel('dep',$event)">
...
export default {
  ...
  methods: {
    populateLists(){
      var self = this
      store.state.territoireData["dep"].forEach(function (level) {
        let depObj = { "label":level["libelle"],"value":level["dep"] }
        self.departements.push(depObj)
      })
      store.state.territoireData["reg"].forEach(function (level) {
        let regObj = { "label":level["libelle"],"value":level["reg"] }
        self.regions.push(regObj)
      })
    },
    changeGeoLevel(level,event){
      level == "dep" ? document.querySelector('#select-reg').value = "" : document.querySelector('#select-dep').value = ""
